...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  % size: <number hidden units> by <number of data cases>
  hidden_delta = (model.hid_to_class' * output_delta) .* hid_output .* (1 - hid_output);
  
  ret.input_to_hid = 1 / N * (hidden_delta * data.inputs') + model.input_to_hid * wd_coefficient;
  ret.hid_to_class = 1 / N * (output_delta * hid_output') + model.hid_to_class * wd_coefficient;
end
